...
export default function MessageThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[60] flex flex-col w-full bg-[#080016] overflow-hidden"
    >
      {/* Dynamic Slide-in DM Header */}
      <div className="flex items-center justify-between p-4 border-b border-unseen-800/30 bg-[#080016]/95 backdrop-blur-md z-10">
        <div className="flex items-center space-x-3 cursor-pointer group" onClick={() => router.push(`/profile?id=${participantId}`)}>
          <button onClick={(e) => { e.stopPropagation(); router.push('/messages'); }} className="p-1.5 -ml-1 text-gray-400 hover:text-white transition-colors">
      ...
      <div className="p-4 border-t border-unseen-800/30 bg-[#080016]/95 pb-[calc(1.2rem+env(safe-area-inset-bottom))]">
