...
export interface FeedContext {
  userMoods: string[];        // moods user has interacted with
  userHashtags: string[];     // hashtags user follows/engages with
  followedAuthors: string[];  // author IDs the user follows
  feedMode: 'for_you' | 'following' | 'trending' | 'late_night' | 'rising';
}
 ...
 */
function calcEngagement(post: IPost): number {
  const raw =
    (post.likesCount * 1) +
 ...
 */
function calcFreshness(createdAt: Date): number {
 ...
 */
function calcRelevance(post: IPost, ctx: FeedContext): number {
  let score = 0;
...
function lateNightBoost(post: IPost): number {
  if (!post.moodTag) return 0;
 ...
 */
function calcVelocity(post: IPost): number {
 ...
 */
export function scorePost(post: IPost, ctx: FeedContext): number {
  ...
  switch (ctx.feedMode) {
    case 'following':
      ...
      if (ctx.followedAuthors.includes(post.author.toString())) score += 30;
      break;
    case 'trending':
      score = E * 0.7 + F * 0.3;
      break;
 ...
 */
export function rankPosts(posts: IPost[], ctx: FeedContext): IPost[] {
  return posts
 ...
 */
export function detectFeedMode(): 'late_night' | 'for_you' {
