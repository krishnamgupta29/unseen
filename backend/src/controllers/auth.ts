...
export const signup = async (req: Request, res: Response) => {
  try {
    ...
    const ipHash = hashOneWay(req.ip || 'unknown');
    ...
    if (finalUsername) {
      ...
      let attempts = 0;
      while (await User.findOne({ username: finalUsername }) && attempts < 10) {
        finalUsername = generateUsername();
        attempts++;
      }
    }
...
export const refresh = async (req: Request, res: Response) => {
  try {
    ...
    await RefreshToken.create({
      ...
      deviceInfo: req.headers['user-agent'] || 'Unknown',
      ipAddress: hashOneWay(req.ip || 'unknown'),
      expiresAt,
    });
...
export const verifyAndLinkEmail = async (req: AuthRequest, res: Response) => {
  try {
...
export const removeEmail = async (req: AuthRequest, res: Response) => {
  try {
